...
![Maite の見た目](site/shots/ja/card.png)

<img src="site/shots/ja/overlay.png" alt="画面の隅に出るオーバーレイ（原寸）" width="200">
...
プロンプトのときだけ `"size":"m"` が 1 つ増える。**これで全部**：
...
| 系統 | 伸ばすツール |
|---|---|
| 職人 | `Bash` `BashOutput` `KillShell` |
| 探索者 | `WebSearch` `WebFetch` `mcp__*` |
| 建築家 | `Write` `Edit` `NotebookEdit` |
| 学者 | `Read` `Grep` `Glob` |
| 統率者 | `Task` `Agent` `Workflow` |
...
<img src="site/shots/ja/name.png" alt="名前の由来と 4 軸" width="360">
...
画面に出ている間、ときどき勝手に動く。伸び・あくび・首かしげ・くるっと回る・うなずく・鼻歌・ため息・つまずく・頭をかく・寝返り ── **手が空いているときだけで 27 通り**あって、そのときの気分でできることが変わる。
...
<img src="site/shots/ja/home.png" alt="スマホから見たところ" width="320">
...
- **Phase 2** … 同ランク帯のユーザーとゴースト戦。練習相手を本物の個体に差し替えるだけで動くところまで来ている。**やるかどうかはまだ決めていない**
